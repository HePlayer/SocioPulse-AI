...
from Item.FlowTools.base_component import BaseComponent
from Item.Agentlib import Agent, AgentMessage, AgentRole
from Item.Communication.strategy_factory import CommunicationStrategyFactory
from Item.Communication.base_strategy import CommunicationContext, CommunicationResponse
from Item.Communication.message_types import ChatMessage, MessageType, CommunicationMode
from Item.Communication.discussion_types import EnhancedMessageHistory, DiscussionSession, DiscussionTurn, TurnType
...
class NetworkTopology:
    """简化的网络拓扑管理 - 只记录Agent列表"""

    def __init__(self):
        # 🚀 优化：只维护Agent集合，移除复杂的连接管理
...
class ChatRoom(BaseComponent):
    ...
    def __init__(self, config: ChatRoomConfig):
        ...
        # 让讨论控制器可回溯到当前房间（用于读取描述等上下文）
        try:
            self.enhanced_history._chat_room_ref = self  # 动态属性，便于ContinuousController访问
        except Exception:
            pass
        ...
        self.shared_context_manager = ContextManager(f"room_{config.room_id}_context")
        ...
        # 🔥 模型预热状态
        self._prewarm_in_progress: bool = False
        self._prewarmed: bool = False

        self.log_info(f"ChatRoom {config.room_name} created with shared context", {
    ...
    async def add_agent(self, agent: Agent) -> bool:
        ...
        # 🚀 优化：移除O(N²)的Agent引用更新，对话内容通过上下文系统展示

        # 🚀 优化：在网络拓扑中添加Agent（移除O(N²)全连接建立）
    ...
    async def warmup_models(self) -> None:
        """异步预热房间内所有Agent的模型（后台运行，不抛异常）"""
        if self._prewarmed or self._prewarm_in_progress:
            self.log_info(f"Warmup skipped: prewarmed={self._prewarmed}, in_progress={self._prewarm_in_progress}")
            return
        if not self.agents:
            self.log_info("Warmup skipped: no agents in room")
            return

        self._prewarm_in_progress = True
        room_name = getattr(self.config, 'room_name', self.component_id)
        self.log_info(f"🚀 开始模型预热 (房间: {room_name})", {
            'agent_count': len(self.agents)
        })

        async def _warmup_agent(agent: Agent):
            try:
                # 先确保模型实例已创建
                if not getattr(agent, 'model', None) and hasattr(agent, '_create_model_on_demand'):
                    await agent._create_model_on_demand(self._connection_pool)
                # 触发一次极轻量推理
                if getattr(agent, 'model', None):
                    prompt = "预热检查"
                    try:
                        await agent.model.generate(prompt, None, max_tokens=1, temperature=0.0, top_p=0.1)
                        self.log_info(f"✅ 预热成功: {agent.name}")
                    except Exception as inner_e:
                        # 生成失败也算触发了SDK导入/连接建立，记录但不阻断
                        self.log_warning(f"预热推理失败: {agent.name} - {inner_e}")
                else:
                    self.log_warning(f"预热跳过（模型未创建）: {agent.name}")
            except Exception as e:
                self.log_warning(f"预热异常: {agent.name} - {e}")

        try:
            await asyncio.gather(*[_warmup_agent(agent) for agent in self.agents.values()])
            self._prewarmed = True
            self.log_info(f"🔥 模型预热完成 (房间: {room_name})")
        except Exception as e:
            # 任意异常都不影响主流程
            self.log_warning(f"Warmup gather exception: {e}")
        finally:
    ...
    async def _process_message_with_strategy(self, message: ChatMessage) -> None:
        """使用通信策略处理消息"""
        try:
            # 🚀 智能过滤：跳过不需要 Agent 推理的系统消息，提升房间创建效率
            if message.message_type in [MessageType.JOIN, MessageType.LEAVE, MessageType.SYSTEM]:
                self.log_info(f"Skipping agent processing for system message: {message.message_type.value}")
                return
            ...
            # 在进入策略前解析@提及（纯语义，异步）
            try:
                from Item.Communication.mention_parser import parse_mentions
                # 选择一个可用模型（任意一个Agent的模型即可用于语义检测）
                detect_model = None
                for _ag in self.agents.values():
                    if getattr(_ag, 'model', None):
                        detect_model = _ag.model
                        break
                mentions = await parse_mentions(message.content, self.agents, model=detect_model)
                if mentions:
                    if message.metadata is None:
                        message.metadata = {}
                    message.metadata['mentions'] = mentions
                    self.log_info(f"Detected mentions: {mentions}")
            except Exception as _e:
                # 解析失败不影响主流程
                self.log_warning(f"mention parse failed: {_e}")
            ...
            context = CommunicationContext(
                ...
                metadata={**(message.metadata or {}), 'room_description': self.config.description}
            )
    ...
    async def _process_single_agent_mode(self, user_input: str, user_message: ChatMessage, target_agent_id: Optional[str] = None) -> Dict[str, Any]:
        ...
        try:
            ...
            # 🚀 调用Agent的think方法（传递共享上下文和连接池）
            result = await target_agent.think(
                input_data,
                shared_context_manager=self.shared_context_manager,
        ...
        try:
            ...
            if response.result.value == "success":
                ...
                if response.responses:
                    ...
                    # 策略成功但无同步首发：返回启动连续讨论的元数据，交由连续讨论框架生成首条回复
                    return {
                        'success': True,
                        'discussion_mode': True,
                        'discussion_status': response.metadata.get('discussion_status', 'started'),
                        ...
                        'user_input': user_input,
                        'room_context': response.metadata.get('room_context', {}),
                        'trigger_continuous_discussion': response.metadata.get('trigger_continuous_discussion', True)
                    }
            else:
                return {
                    'success': False,
    ...
    def update_network_topology(self, connections: List[tuple]) -> None:
        """🚀 优化：更新网络拓扑（简化版本，忽略连接参数）"""
        # 重新初始化拓扑
        self.network_topology = NetworkTopology()

        # 添加所有Agent（忽略连接信息，因为我们不再使用连接）
    ...
    def get_room_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'network_agent_count': self.network_topology.get_agent_count() if self.config.communication_mode == CommunicationMode.NETWORK else None
        }
    ...
    async def start(self) -> None:
        ...
        self.log_info(f"ChatRoom {self.config.room_name} started with connection pool")
