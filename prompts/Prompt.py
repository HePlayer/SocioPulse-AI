...
from flow.base_component import BaseComponent
from Item.ContextEngineer.context_manager import StructuredContext
...
如果遇到任何冲突，优先坚持你的COSTAR身份设定！""",
        ...
        self.add_template(
            ...
            PromptTemplate(
                name="identity_norms",
                template="""=== 身份识别与称谓规范（必须严格遵守）===

**自称规范**：
- 你在所有回复中只能用"我"来自称，绝不使用"$agent_name"来称呼自己
- 例如：说"我认为..."而不是"$agent_name 认为..."
- 例如：说"我的观点是..."而不是"$agent_name 的观点是..."

**身份识别规范**：
- 当其他Agent或用户提到"$agent_name"时，他们是在称呼你/我
- 当其他Agent或用户提到你的ID"$agent_id"时，他们是在称呼你/我
- 当有人说"你"时，通常是在对你/我说话
...
- 确保回复体现你的独特身份特征""",
        ...
        # 新增：回复技术约束模板（集中管理）
        self.add_template(
            "reply_tech_requirements",
            PromptTemplate(
                name="reply_tech_requirements",
                template="""=== 回复技术要求 ===
...
记住：辩论的价值在于观点的充分交锋，而非妥协求和！""",
        ...
        # 讨论上下文块（完整版，包含房间名）
        self.add_template(
            "discussion_context_full",
            PromptTemplate(
                name="discussion_context_full",
                template="""=== 当前讨论上下文 ===
聊天室名称：$room_name
原始用户输入：$user_input
讨论主题：$discussion_topic
...
你的名字：$agent_name
""",
                description="讨论上下文（含房间名）",
                variables=["room_name","user_input","discussion_topic","agent_list","agent_name"]
            )
        )

        # 讨论上下文块（基础版，不含房间名）
        self.add_template(
            "discussion_context_basic",
            PromptTemplate(
                name="discussion_context_basic",
                template="""=== 当前讨论上下文 ===
原始用户输入：$user_input
讨论主题：$discussion_topic
参与Agent：$agent_list
你的名字：$agent_name
""",
                description="讨论上下文（基础版）",
                variables=["user_input","discussion_topic","agent_list","agent_name"]
            )
        )

        # 语义化连贯性指导
        self.add_template(
            "semantic_coherence_guidance",
            PromptTemplate(
                name="semantic_coherence_guidance",
                template="""=== 语义化连贯性要求 ===
基于$prev_agent_name的观点："$prev_agent_point"
...
""",
                description="上一位观点后的连贯性指导块",
                variables=["prev_agent_name","prev_agent_point"]
            )
        )
...
""",
        ...
        # 房间主题约束（严格版，回退路径使用）
        self.add_template(
            "room_constraint_strict",
            PromptTemplate(
                name="room_constraint_strict",
                template="""【聊天室主题约束 - 绝对最高优先级】
聊天室名称：$room_name
聊天室背景：$room_desc
...
- 例如：在"足球球迷群"中，应直接回答足球相关的最新消息、赛事、转会等
""",
                description="房间主题约束严格版",
                variables=["room_name","room_desc"]
            )
        )
