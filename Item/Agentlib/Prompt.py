...
最近讨论内容：$recent_content

=== 语义分析任务 ===
请通过语义理解判断：

1. **主题性质分析**：
   - 这是什么性质的对话？（问候、提问、讨论、争议、总结等）
   - 用户的真实意图是什么？
   - 这个话题需要多深入的讨论？
...
3. **停止必要性判断**：
   - 基于语义理解，这个讨论是否应该停止？
   - 如果是简单问候，是否已经完成了社交功能？
   - 如果是复杂问题，是否需要更多轮次来充分回答？

=== 评分指导 ===
stop_value (停止倾向 0-1)：
- 0.0-0.2：强烈需要继续（复杂问题刚开始、用户明确提问未充分回答）
- 0.3-0.4：适度继续（有价值的讨论正在进行）
- 0.5-0.6：中性（可停可续）
- 0.7-0.8：倾向停止（话题基本完成、开始重复）
- 0.9-1.0：强烈停止（简单问候已完成、明显偏离主题）

value_score (继续价值 0-100)：
- 基于我继续发言能为讨论带来的价值
- 考虑是否有新观点、是否能推进讨论、是否符合用户期望

repeat_risk (重复风险 0-1)：
- 基于语义相似度判断是否会重复之前的内容
- 不是简单的词汇重复，而是观点和信息的重复

请仅输出JSON格式：{"stop_value":0.0-1.0,"value_score":0-100,"repeat_risk":0.0-1.0}""",
                description="SVR计算专用模板（语义理解版本）",
                variables=["agent_name","total_turns","momentum","balance","self_len","others_len","recent_content","user_input"]
            )
        )
