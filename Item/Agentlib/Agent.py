        ...
        try:
            ...
            if self.model:
                try:
                    ...
                    self.log_error(f"Model generation failed: {model_error}")
                    response = f"[{self.name}] 抱歉，我在处理您的请求时遇到了模型调用错误。请检查API配置。错误信息: {str(model_error)}"
            else:
                response = f"[{self.name}] 收到输入: {user_input}\n\n这是一个基础响应，因为没有配置模型。请配置模型以获得智能回复。"
