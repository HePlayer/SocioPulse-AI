...
你是{self.name}，角色是{self.role.value}。
{getattr(self, 'system_prompt', '')}

当前参与讨论的其他Agent：{room_context.get('available_agents', [])}
"""
                        # 将Agent身份信息注入到开发者指令的开头
                        if not hasattr(context, 'developer_instructions'):
                            context.developer_instructions = []
                        context.developer_instructions.insert(0, agent_identity_prompt)
    ...
    def _build_discussion_prompt(self, user_input: str, room_context: Dict[str, Any]) -> str:
        ...
        base_prompt = self.prompt_manager.get_system_prompt() if self.prompt_manager else None
        if base_prompt:
