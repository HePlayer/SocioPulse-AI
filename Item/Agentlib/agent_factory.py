...
class AgentFactory:
    ...
    def _create_basic_agent(self, config: AgentCreationConfig) -> Agent:
        """创建基础Agent（最小配置）- 轻量级模式"""
        # 🚀 轻量级Agent：只存储基本信息，不初始化重型组件
        agent = Agent(
            agent_id=config.agent_id,
            name=config.name,
            role=config.role,
            model=None,  # 不立即创建模型
            context_manager=None,  # 不创建独立的上下文管理器
            prompt_manager=None  # 不创建提示词管理器
        )

        # 存储模型配置用于延迟初始化
        agent._model_config = {
            'platform': config.model_type,
            'model_name': config.model_name,
            'system_prompt': config.system_prompt or config.custom_prompt
        }

        # 存储系统提示词
        agent.system_prompt = config.system_prompt or config.custom_prompt or ""
    ...
    def _create_default_model_config(self, model_type: str, model_name: Optional[str], api_key: str) -> ModelConfig:
        """创建默认模型配置（优先遵循外部配置），避免强制切到 glm-4.5 导致失败"""
        # 1) 优先使用 ConfigManager 中的默认模型
        if not model_name and self.config_manager:
            try:
                cfg_default = self.config_manager.get_default_model(model_type)
                if cfg_default:
                    model_name = cfg_default
            except Exception:
                pass

        # 2) 仍未指定时，使用更保守的内置默认映射（不强制 glm-4.5）
        if not model_name:
            default_models = {
                'openai': 'gpt-3.5-turbo',
                'aihubmix': 'gpt-4o-mini',
                'zhipu': 'glm-4',     # 保守默认，确保基础可用
                'zhipuai': 'glm-4'
            }
        ...
        if self.config_manager:
            try:
                api_base = self.config_manager.get_api_base(model_type)
            except Exception:
                api_base = None
        if not api_base:
            api_bases = {
