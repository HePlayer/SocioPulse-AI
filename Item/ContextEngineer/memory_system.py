...
class MemorySystem(BaseComponent):
    ...
    def add_episodic_memory(self, content: str, metadata: Dict[str, Any] = None, importance_score: float = 1.0) -> str:
        """添加情景记忆"""
        return self.episodic_memory.add_memory(content, metadata, importance_score)
    
    def add_procedural_memory(self, content: str, metadata: Dict[str, Any] = None, importance_score: float = 1.0) -> str:
        """添加程序记忆"""
        return self.procedural_memory.add_memory(content, metadata, importance_score)
    
    def add_semantic_memory(self, content: str, metadata: Dict[str, Any] = None, importance_score: float = 1.0) -> str:
        """添加语义记忆"""
        return self.semantic_memory.add_memory(content, metadata, importance_score)
    
    def search_memories(self, query: str, memory_types: List[MemoryType] = None, top_k: int = 5) -> List[Dict[str, Any]]:
        """搜索记忆"""
        all_results = []
        ...
        if MemoryType.EPISODIC in memory_types:
            all_results.extend(self.episodic_memory.search_memories(query, top_k))
        
        if MemoryType.PROCEDURAL in memory_types:
            all_results.extend(self.procedural_memory.search_memories(query, top_k))
        
        if MemoryType.SEMANTIC in memory_types:
            all_results.extend(self.semantic_memory.search_memories(query, top_k))
        
        # 按相似度和重要性排序
        all_results.sort(key=lambda x: (x.get('similarity_score', 0), x.get('importance_score', 0)), reverse=True)
    ...
    def clear_all_memories(self) -> None:
        """清空所有记忆"""
        self.episodic_memory.memories.clear()
        self.procedural_memory.memories.clear()
        self.semantic_memory.memories.clear()
        self.log_info("All memories cleared")
    ...
    def execute(self, input_data: Any) -> Any:
        ...
        if isinstance(input_data, dict):
            ...
            elif action == 'add_episodic':
                return self.add_episodic_memory(input_data['content'], input_data.get('metadata'), input_data.get('importance_score', 1.0))
            elif action == 'add_procedural':
                return self.add_procedural_memory(input_data['content'], input_data.get('metadata'), input_data.get('importance_score', 1.0))
            elif action == 'add_semantic':
                return self.add_semantic_memory(input_data['content'], input_data.get('metadata'), input_data.get('importance_score', 1.0))
            elif action == 'search':
                return self.search_memories(input_data['query'], input_data.get('memory_types'), input_data.get('top_k', 5))
        return {'error': 'Invalid input'}
