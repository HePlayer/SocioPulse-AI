...
│   ├── assets/                     # 资源文件（css/js/...）
│   └── README.md                   # 前端文档
├── Server/                         # 后端服务器（保留包名，向后兼容）
│   ├── __init__.py                 # 包初始化（导出 SocioPulseServer 兼容 MultiAIServer）
│   ├── main.py                     # 主服务器类（SocioPulseServer）
│   ├── config.py                   # 配置常量（INDEX_FILE_PATH 等）
│   ├── utils.py                    # 工具函数
...
├── models/                         # 模型调用与管理
│   ├── models.py                   # 基础模型与工厂
│   ├── enhanced_models.py          # 增强模型适配器
│   ├── connection_pool.py          # 连接池
│   └── sdk_templates.py            # 请求模板
├── prompts/                        # 提示词与组装
│   └── Prompt.py
├── flow/                           # 流程与组件
│   ├── base_component.py
...
│   ├── chat_room.py
│   ├── mention_parser.py
│   └── discussion_framework/       # 连续讨论框架（新位置）
│       ├── __init__.py             # 统一导入出口
...
│   └── ChatRoom.py
├── scripts/                        # 脚本/演示
├── Server.py                       # 服务器入口（兼容 MultiAIServer 名称）
├── config.yaml                     # 配置文件
...
```python
# 创建讨论框架（新路径，兼容旧路径）
from comm.discussion_framework import DiscussionFrameworkManager
...
result = await manager.start_enhanced_discussion(
    room_id="room_123",
    topic="讨论主题",
    participants=agent_dict,  # Dict[str, Agent]
    initial_message=ChatMessage(sender_id="user", content="开始讨论", message_type=MessageType.TEXT)
)

# 查询状态
status = await manager.get_discussion_status("room_123")

# 控制讨论
await manager.control_discussion("room_123", action="pause")
await manager.control_discussion("room_123", action="resume")
await manager.control_discussion("room_123", action="stop")
```
