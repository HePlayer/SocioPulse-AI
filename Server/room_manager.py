...
class RoomManager:
    ...
    def __init__(self, chat_rooms: Dict, workflow_builder: WorkflowBuilder):
        ...
        self.discussion_framework_manager = None  # 将在服务器初始化后设置
    ...
    async def handle_create_room(self, request):
        ...
        try:
            ...
            # 检查是否启用讨论模式（多Agent群聊时启用）
            discussion_enabled = (room_type == ROOM_TYPES['GROUP'] and len(validated_agents) > 1)
            ...
            # 🚀 优化：轻量级Agent创建
            async def create_single_agent(agent_config):
                """创建单个轻量级Agent的异步函数"""
                try:
                    ...
                    config = AgentCreationConfig(
                        ...
                        creation_mode=AgentCreationMode.BASIC  # 🚀 关键：使用BASIC模式
                    )
                    ...
                    logger.info(f"✅ Lightweight agent created: {agent_config['name']}")
                    return agent

                except Exception as e:
                    logger.error(f"Failed to create lightweight agent {agent_config['name']}: {e}")
                    raise Exception(f"创建Agent '{agent_config['name']}' 失败: {str(e)}")

            # 并行创建所有Agent
            try:
                agent_tasks = [create_single_agent(config) for config in validated_agents]
                created_agents_list = await asyncio.gather(*agent_tasks)
                ...
                for agent in created_agents_list:
                    ...
                    logger.info(f"Added agent {agent.name} to room {room_name}")
            ...
            # ✅ 异步触发模型预热（不阻塞返回）
            try:
                asyncio.create_task(created_room.warmup_models())
                logger.info(f"Warmup task scheduled for room {room_name} ({room_id})")
            except Exception as warm_e:
                logger.warning(f"Failed to schedule warmup for room {room_id}: {warm_e}")
    ...
    async def handle_send_message_to_room(self, request):
        ...
        try:
            ...
            if result.get('success', False):
                # 检查是否启动了增强讨论模式
                if result.get('discussion_mode') and self.discussion_framework_manager:
                    # 如果启动了增强讨论，返回额外的框架信息
                    response_data = create_success_response(
                        message='Enhanced discussion started successfully',
                        agent_response={
                    ...
                    response_data['enhanced_discussion'] = {
                        'session_id': result.get('discussion_session_id'),
                        'framework_active': True,
                        'discussion_status': result.get('discussion_status', 'started')
                    }
