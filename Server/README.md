...
### 1. SocioPulseServer (main.py)（兼容 MultiAIServer）
主服务器类，整合所有功能组件：
- 静态文件服务
- 路由配置
- CORS设置
...
### 环境变量
- `SOCIOPULSE_HOST`（兼容 `MULTIAI_HOST`）: 服务器主机地址 (默认: 0.0.0.0)
- `SOCIOPULSE_PORT`（兼容 `MULTIAI_PORT`）: 服务器端口 (默认: 8080)
...
### 启动服务器
```python
from Server import SocioPulseServer

# 创建服务器实例（仍兼容 MultiAIServer 名称）
