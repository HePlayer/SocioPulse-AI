    ...
    async def _handle_send_message(self, connection_id: str, data: Dict[str, Any]):
        ...
        try:
            ...
            if self.model_manager:
                try:
                    ...
                    if result.get('success', False):
                        # 成功处理，广播Agent响应 - 使用正确的Agent名字
                        correct_agent_name = result.get('agent_name', 'Agent')
                        agent_message = {
                            'type': WS_MESSAGE_TYPES['NEW_MESSAGE'],
                            'room_id': room_id,
                            'sender': correct_agent_name,  # 使用正确的Agent名字而不是'agent'
                            'agent_name': correct_agent_name,  # 在顶层添加agent_name字段
                            ...
                            'message': {
                                ...
                                'sender': correct_agent_name,  # 使用正确的Agent名字
                        ...
                        # 不发送额外的确认消息，Agent名字已经在第一条消息中
                        logger.info(f"Enhanced model manager processing completed successfully")
                        return  # 成功处理，直接返回
            ...
            if hasattr(room, 'process_user_input'):
                ...
                if result.get('success', False):
                    # Agent成功响应，广播Agent的回复 - 使用正确的Agent名字
                    correct_agent_name = result.get('agent_name', 'Agent')
                    agent_message = {
                        'type': WS_MESSAGE_TYPES['NEW_MESSAGE'],
                        'room_id': room_id,
                        'sender': correct_agent_name,  # 使用正确的Agent名字而不是'agent'
                        'agent_name': correct_agent_name,  # 在顶层添加正确的agent_name
                        'content': result.get('response', ''),
                        'timestamp': datetime.now().isoformat(),
                        'message_id': result.get('message_id', str(uuid.uuid4())),
                        'sender_type': 'text',
                        'message': {
                            ...
                            'sender': correct_agent_name,  # 使用正确的Agent名字
                            'sender_id': result.get('agent_id', 'agent'),
                            'agent_name': correct_agent_name,  # 确保message内部也有正确的agent_name
                            'content': result.get('response', ''),
                            'timestamp': datetime.now().isoformat(),
                            'message_type': 'text',
                            'metadata': result.get('metadata', {})
                    ...
                    # 不再发送冗余的message_sent确认消息
                    # Agent名字现在已经正确包含在第一条消息中
                    logger.info(f"Agent response sent successfully, no additional confirmation needed")
                else:
                    # Agent处理失败，发送错误响应
