...
class WebSocketHandler:
    ...
    async def _handle_join_room(self, connection_id: str, data: Dict[str, Any]):
        ...
        if room_id in self.chat_rooms:
            ...
            try:
                ...
                # ✅ 在加入房间后异步触发预热（不阻塞响应）
                try:
                    if hasattr(room, 'warmup_models'):
                        asyncio.create_task(room.warmup_models())
                        logger.info(f"Warmup task scheduled on join for room {room_id}")
                except Exception as warm_e:
                    logger.warning(f"Failed to schedule warmup on join for room {room_id}: {warm_e}")
            except Exception as e:
    ...
    async def _handle_discussion_response(self, room_id: str, result: Dict[str, Any], room):
        ...
        discussion_status = result.get('discussion_status') or discussion_metadata.get('discussion_status')
        ...
        # 新逻辑：取消首发后，discussion_strategy 不再返回同步消息
        # 因此这里不再广播任何首条Agent消息，直接触发连续讨论框架
        if discussion_status == 'started' and (next_turn_ready or trigger_continuous):
            ...
            # 兼容性：若标志缺失但状态启动，则仍触发连续讨论
            await self._trigger_continuous_discussion(room_id, room, result)

        logger.info(f"Discussion handling completed for room {room_id} (no immediate broadcast in discussion mode)")
    ...
    async def _trigger_continuous_discussion(self, room_id: str, room, initial_result: Dict[str, Any]):
        ...
        try:
            ...
            from comm.discussion_framework.agent_id_manager import AgentIDManager
            ...
            _user_input = initial_result.get('user_input', '')
            mentions = []
            try:
                # 选择一个可用模型（任意一个Agent的模型即可用于语义检测）
                detect_model = None
                for _ag in normalized_agents.values():
                    if getattr(_ag, 'model', None):
                        detect_model = _ag.model
                        break
                mentions = await parse_mentions(_user_input, normalized_agents, model=detect_model)
            except Exception as _e:
                logger.debug(f"mention parse failed in trigger: {_e}")
            initial_metadata = {
            ...
            if mentions:
                initial_metadata['mentions'] = mentions
