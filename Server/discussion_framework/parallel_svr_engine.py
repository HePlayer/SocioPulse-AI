...
class AgentSVRComputer:
    ...
    async def _llm_score_svr(self, agent, ctx):
        ...
        try:
            ...
            # 获取SVR评分提示词（添加user_input参数）
            user_input = session_state.get('user_input', '未知输入')
            prompt = pm.get_prompt("svr_scorer", None, agent.metadata,
            ...
            # 调用模型（低温度、小token、短超时）
            response = await m.generate(prompt, None, temperature=0.2, max_tokens=96)
            ...
            if start >= 0 and end > start:
                ...
                svr_data = json.loads(json_str)

                # 范围裁剪
                return {
                    'stop_value': max(0.0, min(1.0, float(svr_data.get('stop_value', 0.5)))),
                    'value_score': max(0.0, min(100.0, float(svr_data.get('value_score', 50.0)))),
                    'repeat_risk': max(0.0, min(1.0, float(svr_data.get('repeat_risk', 0.3))))
                }
            return None
        except Exception:
            return None
