...
class AgentSVRComputer:
    ...
    async def _llm_score_svr(self, agent, ctx):
        """使用Agent的模型计算SVR值"""
        try:
            ...
            # 准备模板变量
            agent_turns = ctx.get('agent_turns', [])
            other_turns = ctx.get('other_turns', [])
            session_state = ctx.get('session_state', {})
            global_metrics = ctx.get('global_metrics', {})

            # 计算特征值
            self_len = sum(len(t.message.content) for t in agent_turns[-1:]) if agent_turns else 0
            others_len = sum(len(t.message.content) for t in other_turns[-1:]) if other_turns else 0
            ...
            if all_turns:
                # 按时间排序，取最近2条
                sorted_turns = sorted(all_turns, key=lambda t: getattr(t, 'timestamp', 0))
                recent_turns = sorted_turns[-2:]
                recent_content = " ".join([t.message.content[:50] for t in recent_turns])  # 限制长度

            # 构造最近历史（更长上下文给LLM评分参考）
            recent_history = []
            for t in sorted(all_turns, key=lambda x: getattr(x, 'timestamp', 0))[-6:]:  # 最近最多6条
                who = getattr(t, 'agent_name', t.agent_id)
                recent_history.append(f"{who}: {t.message.content[:120]}")
            recent_history_text = "\n".join(recent_history)

            # 注入Agent身份与能力信息
            metadata = getattr(agent, 'metadata', None)
            identity_name = getattr(agent, 'name', '')
            identity_role = getattr(agent, 'role', '')
            identity_desc = getattr(metadata, 'description', '') if metadata else ''
            system_prompt = ''
            capabilities = []
            constraints = []
            custom_attrs = {}
            if metadata:
                if hasattr(metadata, 'custom_attributes'):
                    custom_attrs = metadata.custom_attributes or {}
                    system_prompt = str(custom_attrs.get('system_prompt', '') or '')
                capabilities = getattr(metadata, 'capabilities', []) or []
                constraints = getattr(metadata, 'constraints', []) or []

            # 获取SVR评分提示词（扩展上下文注入）
            user_input = session_state.get('user_input', '未知输入')
            prompt = pm.get_prompt(
                "svr_scorer", None, agent.metadata,
                agent_name=identity_name,
                agent_role=getattr(identity_role, 'value', str(identity_role)),
                agent_description=identity_desc,
                system_prompt=system_prompt,
                capabilities=", ".join(capabilities),
                constraints=", ".join(constraints),
                total_turns=session_state.get('total_turns', 0),
                momentum=global_metrics.get('discussion_momentum', 0.5),
                balance=global_metrics.get('participation_balance', 1.0),
                self_len=self_len,
            ...
            # 调用模型（禁用联网功能，避免SVR计算中错误联网）
            response = await m.generate(prompt, None,
                                      temperature=0.2,
                                      max_tokens=96,
            ...
            try:
                # 尝试直接解析
                svr_data = json.loads(response.content.strip())
                ...
                if start >= 0 and end > start:
                    try:
                        svr_data = json.loads(content[start:end])
                    except json.JSONDecodeError:
                        return None
                else:
                    return None
            ...
            # 验证并规范化数据
            if isinstance(svr_data, dict):
                def _to_unit_interval(val):
                    """转换为[0,1]区间"""
                    try:
                        f_val = float(val)
                        return max(0.0, min(1.0, f_val / 100.0 if f_val > 1.0 else f_val))
                    except (ValueError, TypeError):
                        return 0.5

                def _to_hundred_scale(val):
                    """转换为[0,100]区间"""
                    try:
                        f_val = float(val)
                ...
                s_raw = svr_data.get('stop_value', 50)
                v_raw = svr_data.get('value_score', 50)
                r_raw = svr_data.get('repeat_risk', 30)

                return {
                    'stop_value': _to_unit_interval(s_raw),
                    'value_score': _to_hundred_scale(v_raw),
                    'repeat_risk': _to_unit_interval(r_raw)
                }
            return None
        except Exception:
            return None
    ...
    async def compute_svr(self, context: Dict[str, Any]) -> AgentSVRResult:
        ...
        try:
            ...
            # 优先尝试LLM打分，失败则回退启发式
            agent_obj = context.get('agent_ref')
            svr_json = None
            try:
                if agent_obj:
                    svr_json = await self._llm_score_svr(agent_obj, context)
            except Exception:
                svr_json = None

            # 计算原始SVR值（LLM优先，失败回退启发式）
            if svr_json:
                computed_stop_value = float(svr_json.get('stop_value', 0.5))
                computed_value_score = float(svr_json.get('value_score', 50.0))
                computed_repeat_risk = float(svr_json.get('repeat_risk', 0.3))
            else:
                # 回退到启发式计算
...
class ParallelSVREngine:
    ...
    async def compute_parallel_svr(self,
        ...
        for agent_id in participants.keys():
            ...
            agent_context['agent_ref'] = participants[agent_id]  # 注入Agent引用用于模型调用
    ...
    def _calculate_global_metrics(self, agent_results: Dict[str, AgentSVRResult]) -> Dict[str, float]:
        ...
        # 计算停止趋势（平均stop_delta）
        stop_deltas = [r.svr_values.get('stop_delta', 0.0) for r in agent_results.values()]
        global_stop_trend = sum(stop_deltas) / len(stop_deltas) if stop_deltas else 0.0
        ...
        return {
            ...
            'global_stop_trend': global_stop_trend,  # 停止趋势
    ...
    def _determine_next_action(self,
        ...
        # 检查停止条件
        global_stop = global_metrics.get('global_stop_average', 0.0)
