...
# 优先使用新命名（SOCIOPULSE_*），兼容旧命名（MULTIAI_*）
HOST = os.getenv('SOCIOPULSE_HOST', os.getenv('MULTIAI_HOST', _server_config.get('host', DEFAULT_HOST)))
PORT = int(os.getenv('SOCIOPULSE_PORT', os.getenv('MULTIAI_PORT', _server_config.get('port', DEFAULT_PORT))))
...
# 默认模型配置
# 将 zhipu 默认模型从免费模型调整为 glm-4.5，以便在未显式指定模型时优先使用 4.5 系列
DEFAULT_MODELS = {
    ...
    'models': {
        ...
        'platforms': {
            ...
            'zhipu': {
                ...
                'enabled_models': ['glm-4-flash-250414', 'glm-3-turbo', 'glm-4', 'glm-4.5'],  # 免费模型优先
...
INDEX_FILE_PATH = 'MultiAI.html'  # 文档命名统一不影响前端文件名
